...
from utils import PUNCTUATION_LIST, uniform, is_chinese_string, edit_distance_word, ErrorType, is_alphabet_string, is_name, need_correct_pinying, filter_entity
from config import config
from seg_utils import Tokenizer
from company import get_query_entity, get_entity
#from ner_detect import nerdetect
...
class Detector(Tokenizer):
    ...
    def _get_maybe_error_index(scores, ratio=0.6745, threshold=1.4):
        ...
        if len(scores.shape) == 1:
            scores = scores[:, None]
        median = np.mean(scores);#np.median(scores, axis=0)  # get median of all scores
        ...
        maybe_error_indices = np.where(scores < median) #np.where((y_score > threshold) & (scores < median))
        # 取全部疑似错误字的index
    ...
    def detect(self, sentence):
        ...
        if self.is_word_error_detect and len(word_seg) < 3:
...
class Corrector(Detector):
    ...
    def correct(self, sentence):
        ...
        # NER 模型检测错误位置
        #ner_res, ner_error = self.nd.detect(sentence)
        #for e in ner_error:
        #    self._add_maybe_error_item(e, self.maybe_errors)
        # trick: 类似翻译模型，倒序处理
        ...
        for item, begin_idx, end_idx, err_type in self.maybe_errors:
            ...
            if err_type == ErrorType.confusion:
                ...
                if not is_chinese_string(item) or filter_entity(item, before_sent, after_sent): #or is_name(item):
                    continue
...
if __name__ == "__main__":
    try: que = sys.argv[1]
    except: que = "C/S" #"百读jaca开法工成师,andrid程序开发"
    c = Corrector()
