...
LM = os.path.join(os.path.dirname(__file__), '.', 'arpa', 'query_word.3gram.arpa')
model = kenlm.LanguageModel(LM)
print('{0}-gram model'.format(model.order))

sentence = "前 段 工 程 师"   #'language modeling is fun .'
print(sentence)
...
for i, (prob, length, oov) in enumerate(model.full_scores(sentence)):
    print('{0} {1}: {2}'.format(prob, length, ' '.join(words[i+2-length:i+2])))     ; a=' '.join(words[i+2-length:i+2])
    if oov:
...
if __name__ == "__main__":
    a=model.perplexity(' '.join(list("前段工程师"))) ;d=' '.join(list("前 段 工 程 师"))
    aa=model.perplexity(' '.join(list("前端工程师")))
    s=model.score(' '.join(list("前 段 工 程 师")))
    ss=model.score(' '.join(list("前 端 工 程 师")))
    pass