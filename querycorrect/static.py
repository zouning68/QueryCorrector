...
#from debug import parse_line_querys
...
#hadoop_input_file = 'file:///D:/Python Project/query_correct_0/data/search_data.log1'
...
def static_ngram(input_file, output_file):
    try:
        os.system("hadoop fs -rmr " + output_file)
...
def static_query(input_file, output_file):
    try:
        os.system("hadoop fs -rmr " + output_file)
        conf = SparkConf().setAppName("miniProject").setMaster("local[*]")
        sc = SparkContext.getOrCreate(conf)
...
if __name__ == '__main__':
    #init()
    static_query(hadoop_input_file, 'hdfs:///user/kdd_zouning/candidate_query1')
